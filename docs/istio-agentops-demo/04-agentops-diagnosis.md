...
수동 진단(문서 03)에서 사람이 도구 네다섯 개를 오가며 하던 상관분석을, awsops 대시보드가 한 번의 질문으로 대신한다.
내부적으로 incident 상관분석 엔진이 Mimir(메트릭), Loki(로그), Tempo(트레이스), K8s 이벤트, Istio 구성을 동시에 조회해 종합한다.

## 사전 조건

대시보드가 세 신호를 읽으려면 데이터소스가 등록되어 있어야 한다.

- awsops 대시보드 설정에 Mimir, Loki, Tempo 데이터소스 등록
...
```
productpage가 지금 장애야. Metric/Log/Trace를 종합해서 원인을 계층별로 찾아줘.
```

## 기대 응답

incident 엔진이 세 신호와 구성을 종합해 두 개의 독립 원인을 계층과 함께 제시한다.
...
한 번의 질문으로 세 신호가 통합되고, 계층 구분과 수정 방법까지 함께 나온다.

## 수동 방식과의 대비

| 항목 | 수동 (문서 03) | AgentOps |
|---|---|---|
| 도구 수 | Kiali + Grafana(Mimir/Loki/Tempo) + kubectl | 대시보드 하나 |
| 질의 | PromQL, LogQL, TraceQL 직접 작성 | 자연어 한 문장 |
...
- 검증 시 대시보드의 incident 엔진이 실제로 Mimir/Loki/Tempo를 조회하는지(데이터소스 연결 상태) 사전에 확인해 둔다.
