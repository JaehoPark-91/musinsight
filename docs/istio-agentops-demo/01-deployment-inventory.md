...
> 각 파드에 **istio-proxy(Envoy) 사이드카**가 자동 주입됨 → 메트릭·로그·트레이스의 원천.
...
| Trace | **Tempo** | 트레이스 저장·조회 (TraceQL) | 1 |

### D. 수집기 (`observability`)
| 컴포넌트 | 역할 | 파드 |
|---|---|---|
| **Grafana Alloy** | Envoy 메트릭→Mimir, 로그→Loki, 트레이스→Tempo (3종 통합 수집) | ~3 (DaemonSet) |

### E. 수동 진단 UI (`istio-system`)
| 도구 | 용도 | 파드 |
|---|---|---|
| **Kiali** | 메시 서비스 그래프 (엣지별 에러율·mTLS) | 1 |
| **Grafana** | Metric/Log/Trace 통합 뷰어 (Mimir·Loki·Tempo 데이터소스) | 1 |

**신규 합계 ≈ 16 파드** → 기존 16 + 16 = **~32 / 51** (워커 3대, 여유 있음)
...
```
[생산]          [수집]        [저장·조회]       [소비 / UI]
Istio/Envoy ─metric→ Alloy ──→ Mimir ──┐
            ─log───→ Alloy ──→ Loki  ──┼─→ Grafana (수동)  ← 사람이 탭 넘기며
            ─trace─→ Alloy ──→ Tempo ──┘─→ awsops 대시보드 (AgentOps) ← 한 질문
...
## 5. 설정/배선 체크 (파드 외 필수)
- Istio Telemetry: **액세스로그 ON**, **트레이싱 → Tempo(OTLP via Alloy)**, **샘플링 100%**
- Grafana 데이터소스: Mimir / Loki / Tempo 등록
- **awsops 대시보드 데이터소스: Mimir / Loki / Tempo 등록** ← AgentOps incident 엔진이 읽음
- OpenCost Prometheus는 그대로 (비용 전용)

## 6. 리소스/용량 메모
- 저장 = **emptyDir**(임시). 파드 재시작 시 데이터 소멸 (데모엔 무방).
- 데이터 = **트래픽량에 비례**. 트래픽 on-demand → 유휴 누적 ≈ 0.
- Loki/Tempo retention 6h (보험).
