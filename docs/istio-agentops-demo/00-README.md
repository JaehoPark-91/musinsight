...
- 방식 1: 사람이 Metric/Log/Trace 도구를 각각 열어 수동으로 원인을 찾는다.
- 방식 2: AgentOps(awsops 대시보드)가 한 번의 질문으로 세 신호를 통합해 진단한다.
...
| 03 | [수동 진단 방법](03-manual-diagnosis.md) | Kiali, Grafana(Metric/Log/Trace), kubectl로 진단 |
...
Istio/Envoy --metric--> Alloy --> Mimir  --+
            --log-----> Alloy --> Loki   --+--> 수동: Grafana, Kiali
            --trace----> Alloy --> Tempo  --+--> AgentOps: awsops 대시보드
...
- 저장: emptyDir(임시), Trace 샘플링 100%, 트래픽 on-demand
- 네임스페이스: istio-system(메시, Kiali, Grafana), observability(Alloy, Mimir, Loki, Tempo), bookinfo(앱), opencost(기존 비용)
