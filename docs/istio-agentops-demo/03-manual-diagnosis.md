...
Metric, Log를 각각 다른 화면에서 보고 시간축을 맞춰 상관관계를 추론해야 한다.
...
네 개 화면(Kiali, Grafana Mimir/Loki, kubectl)의 결과를 시간축으로 맞춰 종합하면 다음 결론에 도달한다.
...
도구 서너 개를 오가며 PromQL, LogQL을 손으로 작성하고, 각 화면의 타임스탬프를 맞춰가며 사람이 직접 상관관계를 추론해야 한다. 원인이 여러 개이고 계층이 다르면 시간이 더 걸린다. 이 지점이 AgentOps와의 대비 포인트다(문서 04).
