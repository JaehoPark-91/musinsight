...
**Stats**: 40 pages / 54 routes / 25 SQL query files / 16 API routes / 125 MCP tools (8 Gateways) / 18 components
...
- **AI-Powered Analysis** -- 11-route intent classification routes questions to 8 specialized AgentCore Gateways (125 MCP tools) with multi-route parallel execution and SSE streaming
- **External Datasource Integration** -- Connect Prometheus, Loki, Tempo, ClickHouse, Jaeger, Dynatrace, and Datadog with SSRF-protected query execution and AI-powered natural language to PromQL/LogQL/SQL generation
...
- **AI Comprehensive Diagnosis** -- 15-section infrastructure analysis with Bedrock Opus, export to DOCX/Markdown/PDF, scheduled auto-diagnosis
...
User Question
    ...
    |-- Prometheus, Loki, Tempo...   --> Datasource (external observability platforms)
...
| `datasources[]` | Array of external datasource configs (Prometheus, Loki, etc.) | `[]` |
| `datasourceAllowedNetworks[]` | Allowed private CIDRs/hostnames for datasource SSRF allowlist | `[]` |
...
│   │   ├── datasources/            # External datasource management + Explore
│   │   ├── ai-diagnosis/           # AI comprehensive diagnosis + print report
...
│   │   ├── datasource-client.ts    # External datasource HTTP client (7 platforms)
│   │   └── datasource-registry.ts  # Datasource type registry + prompts
...
**현황**: 40 페이지 / 54 라우트 / 25 SQL 쿼리 파일 / 16 API 라우트 / 125 MCP 도구 (8 Gateway) / 18 컴포넌트
...
- **AI 기반 분석** -- 11단계 의도 분류로 8개 전문 AgentCore Gateway(125 MCP 도구)에 질문을 라우팅하며, 멀티 라우트 병렬 실행과 SSE 스트리밍을 지원합니다
- **외부 데이터소스 연동** -- Prometheus, Loki, Tempo, ClickHouse, Jaeger, Dynatrace, Datadog 연결과 SSRF 보호 쿼리 실행, AI 기반 자연어 → PromQL/LogQL/SQL 생성을 지원합니다
...
- **AI 종합 진단** -- Bedrock Opus 기반 15섹션 인프라 분석, DOCX/Markdown/PDF 내보내기, 자동 진단 스케줄링을 지원합니다
...
사용자 질문
    ...
    |-- Prometheus, Loki, Tempo...    --> Datasource (외부 관측성 플랫폼)
...
| `datasources[]` | 외부 데이터소스 설정 배열 (Prometheus, Loki 등) | `[]` |
| `datasourceAllowedNetworks[]` | 데이터소스 SSRF allowlist (사설 CIDR/호스트명) | `[]` |
...
│   │   ├── datasources/            # 외부 데이터소스 관리 + Explore
│   │   ├── ai-diagnosis/           # AI 종합 진단 + 인쇄용 리포트
...
│   │   ├── datasource-client.ts    # 외부 데이터소스 HTTP 클라이언트 (7종 플랫폼)
│   │   └── datasource-registry.ts  # 데이터소스 타입 레지스트리 + 프롬프트
